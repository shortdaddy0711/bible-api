...
class BibleAgent:
    ...
    def run(self, user_message: str, history: List[Dict[str, str]] = []) -> ChatResponse:
        tools = [
            {
                "type": "function",
                "function": {
        ...
        # First pass: LLM decides if it needs tools
        ...
        if tool_calls:
            ...
            for tool_call in tool_calls:
                ...
                thought_process.append(f"Searching for: {function_args.get('query') or f'{function_args.get('book')} {function_args.get('chapter')}'}")
                
                if function_name == "search_bible":
                    result = self.search_bible_tool(**function_args)
                elif function_name == "get_bible_text":
                    result = self.get_bible_text_tool(**function_args)
                elif function_name == "find_pastor_quotes":
                    result = self.find_pastor_quotes_tool(**function_args)
                else:
                    result = "Unknown tool"
            ...
            # Second pass: LLM synthesizes the final answer
